...
class InstallRequirement(object):
    ...
    def uninstall(self, auto_confirm=False):
        ...
        if (egg_info_exists and dist.egg_info.endswith('.egg-info') and
            ...
            paths_to_remove._add(dist.egg_info)
            ...
            # of pip
                ...
                for top_level_pkg in [
                    ...
                    paths_to_remove._add(path)
                    paths_to_remove._add(path + '.py')
                    paths_to_remove._add(path + '.pyc')
                    paths_to_remove._add(path + '.pyo')
            ...
            paths_to_remove._add(distutils_egg_info)

        elif dist.location.endswith('.egg'):
            # package installed by easy_install
            # We cannot match on dist.egg_name because it can slightly vary
            # i.e. setuptools-0.6c11-py2.6.egg vs setuptools-0.6rc11-py2.6.egg
            paths_to_remove._add(dist.location)
            ...
            paths_to_remove._add(develop_egg_link)
        ...
        if dist.has_metadata('scripts') and dist.metadata_isdir('scripts'):
            for script in dist.metadata_listdir('scripts'):
                if dist_in_usersite(dist):
                    ...
                    bin_dir = bin_py
                paths_to_remove._add(os.path.join(bin_dir, script))
                if WINDOWS:
                    paths_to_remove._add(os.path.join(bin_dir, script) + '.bat')
            ...
            if config.has_section('console_scripts'):
                for name, value in config.items('console_scripts'):
                    if dist_in_usersite(dist):
                        ...
                        bin_dir = bin_py
                    paths_to_remove._add(os.path.join(bin_dir, name))
                    if WINDOWS:
                        paths_to_remove._add(
